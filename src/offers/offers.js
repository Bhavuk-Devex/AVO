...
const createOrUpdateOffer = async (req, res) => {
  ...
  try {
    ...
    if (offer_id) {
      ...
      await query(
        "UPDATE offers SET title = ?, description = ?, image = ?, discount_percentage = ?, price = ?, start_date = ?, end_date = ? WHERE id = ?",
        [
          title,
          description,
          image,
      ...
      const result = await query(
        "INSERT INTO offers (business_id, title, description, image, discount_percentage, price, start_date, end_date) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
        [
          business_id,
          title,
          description,
